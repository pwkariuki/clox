...
// return a Value from the stack without popping
static Value peek(int distance) {
    return vm.stackTop[-1 - distance];
}

// initialize next CallFrame on the stack
...
static bool call(ObjClosure* closure, int argCount) {
    ...
    if (vm.frameCount == FRAMES_MAX) { // deep call chain
    ...
    frame->ip = closure->function->chunk.code; // beginning of function's bytecode
    frame->slots = vm.stackTop - argCount - 1;
    return true;
}

// verify callee is callable and initialize a new CallFrame
    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            ...
            case OBJ_CLASS: {
                ...
                } else if (argCount != 0) { // no `init()` but provided arguments
                  runtimeError("Expected o arguments but got %d.", argCount);
                }
                return true;
            }
            case OBJ_CLOSURE:
...
// runtime implementation of OP_INVOKE combining OP_GET_PROPERTY and OP_CALL
...
// check if a method invocation succeeds
static bool invoke(ObjString* name, int argCount) {
    ...
    // look for field called name before looking up method
    Value value;
...
// finds method and places it on the stack
...
// read and run a single bytecode instruction
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_RETURN: {
                ...
                // discard slots callee used for parameters and locals
...
InterpretResult interpret(const char* source) {
    ...
    pop();
    push(OBJ_VAL(closure));
    call(closure, 0); // set up first frame for executing the top-level code
